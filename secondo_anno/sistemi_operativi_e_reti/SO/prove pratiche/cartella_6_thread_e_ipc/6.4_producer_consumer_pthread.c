...
void *producer(void *ptr) {
  ...
  for (i = 1; i <= MAX; i++) {
    pthread_mutex_lock(
        &the_mutex); /* Ottiene l'accesso esclusivo al buffer, quel thread ha
                        esclusivo accesso, gli altri vengono bloccati*/
    ...
    ricontrolli effettivamente la condizione. cioè che non il mutex non si
    aoccupato di nuovo, se libero, salta il while
    */
    while (buffer != 0) {
      pthread_cond_wait(
          &condp,
          &the_mutex); /*due op atomicamente, rilasciato il mutex mettendomi in
                          attesa, gli altri possono entrare e lavorare */
    }
