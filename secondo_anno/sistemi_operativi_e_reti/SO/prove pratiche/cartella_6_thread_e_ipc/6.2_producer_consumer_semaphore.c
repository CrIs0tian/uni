...
binary_output (nome del file.exe che creiamo)
...
#include <pthread.h>
#include <semaphore.h> //implementa la variabile semaforo e le sue operazioni ( up e down)
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
/* Dichiarazione dei semafori per thread*/
...
void down(sem_t *sem) {
  sem_wait(sem); // la controparte di down nel libro
}

void up(sem_t *sem) {
  sem_post(sem); // controparte di up nel libro
}
...
void *producer(void *arg) {
  while (TRUE) {
    ...
    print_buffer(); // Stampa il contenuto del buffer, se lo stampo dopo il full
                    // stampa un buffer diverso da quello che il producer aveva
                    // scritto
...
int main() {
  ...
  // Inizializzazione dei semafori, dalla libreria semaphore.h, con parametri
  sem_init(&mutex, 0, 1);
  sem_init(&empty, 0, N);
  sem_init(&full, 0, 0);
  ...
  sem_destroy(&mutex); // liberazione risorse
  sem_destroy(&empty);
  sem_destroy(&full);
