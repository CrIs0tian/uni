...
int *somme_parziali(int x[], int n) {
  ...
  return s; // la memoria allocata con malloc non viene rilasciata al termine
            // della funzione
}
...
int main() {
  ...
  printf("%p\n", x); /* %p per formattare una variabile puntatore */
  ...
  printf(
      "%f\n",
      *x); /* stampa dell'elemento puntato da x (QUANDO SI STAMPA TRA PARENTESI,
              NELL'ASSEGNAZIONE DELLE VARIABILI INVECE FUNZIONA AL CONTRARIO)*/
