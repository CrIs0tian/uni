#include <stdio.h>
#include <math.h> // libreria per funzioni matematiche, fabs( valore assoluto) è una di queste
...
void test_char(){ //N.B void è una funzione che non ritorna nulla
	char c;
	
	for(c = 'a'; c <= 'z'; c++)
		printf("Il codice ASCII di %c e' %d\n", c, c); // %, sottostringa di formattazione ---> %d, interi / %c, char / %f, float
}
...
void main(){
	...
	for( x = 2; x < 12; x++){ // concettualmente simile al ciclo while  
 ...
 * 		>, <, <=, >=
 * 
 * funzione fabs(x) = valore assoluto di x  (|x|)
 * 
 * */
