...
def draw(N, P):
    ...
    x = list(zip(N,P)) #zippato in liste con coppie come elementi 
    bubble_sort( x )
    
    # x è ordinata rispetto il secondo elemento delle
    # coppie che lo compongono
            
    N, P = list(zip(*x)) #con *x dentro zip eso viene spacchettato (unzip)
...
bubble_sort2(x, ident)

print(x)

# In[]

def bubble_sort3( a, key=None ):
    '''
    a: lista di n tuple (nome, posizione) dove nome è un
        str di lunghezza 1, e posizione un intero
...
bubble_sort3(key=sum, a=x) # per nome
