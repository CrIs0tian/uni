...
def count_int( x ):
    ...
    for e in x:
        if type(e) == int:
            c += 1
        elif type(e) in [list, tuple]: #questo per scrivere se verfiichare che un TIPO di vaariabile appartenga o no a al gruppo a desta (in questo caso [list, tuple])
            c += count_int( e )
    return c
