...
t = hist(
    0, 5, 3, 10, -4, 5, -12, 6, 0
)  # se usata una varibaile, t diventa una tupla con i valori di return come elementi
...
x0, x1, x2, x3 = (
    t  # unpacking = prendiamo gli elementi di una tupla e li assegnamo a delle variabili
)
...
def str_cmp2(x, y, key=lambda x: x):
    """
    Input: x, y, due stringhe; key una funzione con input str
    Output: ritorna -1 se key(x) < key(y); 0 se key(x) è uguale a key(y);
        +1 se key(y)< key(x)
