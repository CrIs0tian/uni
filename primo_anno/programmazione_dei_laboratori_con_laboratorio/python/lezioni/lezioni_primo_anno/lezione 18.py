...
def dizionario_iniziali(a):
    ...
    for x in a:  # per n volte
        ...
        if (
            k in d
        ):  # O(1), perchè stiamo letteralmente cercando una chiave, che come l'inserimento o la lettura ha costo costante
    ...
    for filename in os.listdir(cartella):
        if filename.split(".")[-1] == "csv":
            ...
            for line in f:  # per ogni riga di tutti i file (per n volte)
                ...
                # cosa c'è  che non va? con l'uguale fai un'alias, quindi non pui usare la funzione append
