...
# il + per mantenere gli apici '
print("Il numero di vocali in " + x + " è " + str(numero_vocali))

# In[] esercizio: contare il numero di vocali
# nella stringa x nelle posizioni pari
...
while p < len(x):  # si può fare direttamente cosi senza dover ++ di 1 ogni volta
    if x[p] in "aeiouAEIOU":
        numero_vocali = numero_vocali + 1
        # oppure numero_vocali += 1
    p += 2  # equivalente a p = p+1
...
a = "programmazione"
a[2] = "*"  # NO, le stringhe sono immutabili, non possono essere modificate
# cioè assegnare nuovi valori
