...
def find_in_file(filename,k):
    ...
    for line in f: #eseguito O(n)
        if k in line: #se len(line) é O(1), O(1) operzione costante 
            lines+=(r,) #per concatenare da tupla a tupla, e il suo costo computazionale è O(len(lines)+1),  caso peggiore=O(n^2)
        r+=1
    f.close()
    return lines
...
def find_in_file(filename,k):
    ...
    r=1 #riga corrente
    for line in f: #eseguito O(n)
        if k in line: #se len(line) é O(1), ricerca lineare nel caso peggiore, ma len(line) = O(1), quindi operzione costante 
            lines.append(r)# O(1)
        r+=1
    f.close() #O(1)
    return tuple(lines) #cosi rispettiamo le regole dell'output, ma a costo O(n) (la conversione costa)
t =find_in_file('lezione 17.py','lines')
print(t)
#COSTO COMPUTAZIONALE
...
def find_in_file(filename,k):
    ...
    for line in f: #eseguito O(n)
        for c in range(len(line)-len(k)+1): #costa O(len(line)-len(k)) sempre
            if k == line[c:c+len(k)]:#len(line)-len(k)[uguale a c]+len(k) ultimo punto che ci può controllare
